...
class BookingServiceImplTest {
    ...
    void getAllByBookerAndStatus() throws Exception {
        ...
        when(bookingRepository.findAllByBookerIdAndStatus(any(Long.class), any(Status.class), any(PageRequest.class)))
                .thenAnswer(
                        invocation -> {
    ...
    void getByOwner() throws Exception {
        ...
        when(bookingRepository.findAllByItemOwnerIdAndStatus(any(Long.class), any(Status.class), any(PageRequest.class)))
                .thenAnswer(
                        invocation -> {
                            Long userId = invocation.getArgument(0, Long.class);
                            Status status = invocation.getArgument(1, Status.class);
